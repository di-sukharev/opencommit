...
const translation = i18n[(config?.OPENCOMMIT_LANGUAGE as I18nLocals) || 'en'];
...
${config?.OPENCOMMIT_EMOJI? 'Use GitMoji convention to preface the commit.': 'Do not preface the commit with anything.'}
${config?.OPENCOMMIT_DESCRIPTION  ? 'Add a short description of WHY the changes are done after the commit message. Don\'t start it with "This commit", just describe the changes.': "Don't add any descriptions to the commit, only commit message."}
...
app.use((_, res, next) => {
 ...
 +  console.log(\`Server listening on port \${PORT}\`);
  ...
  },
  {
    ...
    content: `${config?.OPENCOMMIT_EMOJI ? '🐛 ' : ''}${translation.commitFix}
${config?.OPENCOMMIT_EMOJI ? '✨ ' : ''}${translation.commitFeat}
${config?.OPENCOMMIT_DESCRIPTION ? translation.commitDescription : ''}`
  }
];
